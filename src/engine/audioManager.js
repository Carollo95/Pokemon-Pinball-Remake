...
import {MUSIC_VOLUME, SFX_VOLUME} from "./engine.js";
import {pad3} from "./assetManager.js";
...
class AudioManager {
    ...
    playMusic(id, { restart = true, fade = false } = {}) {
        ...
        introSource.onended = () => {
            ...
            loopSource.onended = () => {
                try { loopSource.disconnect(); } catch { /* empty */ }
                try { gainNode.disconnect(); } catch { /* empty */ }
            };
        };
    }
    ...
    stopMusic({ fade = true } = {}) {
        ...
        const doStopAndDisconnect = () => {
            if (music.introSource) {
                try { music.introSource.stop(); } catch { /* empty */ }
                try { music.introSource.disconnect(); } catch { /* empty */ }
            }
            if (music.loopSource) {
                try { music.loopSource.stop(); } catch { /* empty */ }
                try { music.loopSource.disconnect(); } catch { /* empty */ }
            }
            try { music.gainNode.disconnect(); } catch { /* empty */ }
        };
    ...
    playSFX(id, cooldown = 0, callback = undefined) {
        ...
        source.onended = () => {
            try { source.disconnect(); } catch { /* empty */ }
            try { gainNode.disconnect(); } catch { /* empty */ }
    ...
    playSFXsequence(ids) {
        return ids.reduce((p, id) => p.then(() => new Promise(resolve => {
            ...
            src.onended = () => { try { src.disconnect(); } catch { /* empty */ } try { g.disconnect(); } catch { /* empty */ } resolve(); };
            src.start(0);
        })), Promise.resolve());
    }
    ...
    dispose() {
        ...
        try { this.musicGain.disconnect(); } catch { /* empty */ }
        try { this.sfxGain.disconnect(); } catch { /* empty */ }
        ...
        if (this._lastSfxTime) {
            try { this._lastSfxTime.clear(); } catch { /* empty */ }
        }

        if (this.audioContext && this.audioContext.state !== 'closed') {
            try { this.audioContext.close(); } catch { /* empty */ }
        }
