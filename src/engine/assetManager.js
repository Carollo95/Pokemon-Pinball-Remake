import {DEBUG} from "./cheatEngine.js";
import {BASIC_POKEMON} from "../core/dex.js";
...
export function preloadAnimations() {
  ...
  Asset.registerAnimationTemplate('blueFieldEvolutionArrows', 'assets/img/blue-field/evolution_arrows', 30, 50, 5);
...
if (typeof window !== 'undefined') {
  if (window.__ASSET_SINGLETON__ && window.__ASSET_SINGLETON__ !== Asset) {
    try { window.__ASSET_SINGLETON__.dispose(); } catch { /* empty */ }
  }
  window.__ASSET_SINGLETON__ = Asset;

  window.addEventListener('pagehide', () => {
    try { Asset.dispose(); } catch { /* empty */ }
  }, { once: true });
}
