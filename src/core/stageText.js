import {CHAR_SIZE, SEPARATOR_SIZE, StageCharacter} from "./stageCharacter.js";
...
const TEXT_SCROLL_THRESHOLD_MILLIS = 50; // millis between movement while showing text
export const DEFAULT_TEXT_PERSISTENCE_MILLIS = 3000; //Default millis to keep on screen the shown text
...
export class StageStatusBanner {
    ...
    createTextSprites(x, y) {
        for (let i = 0; i <= this.getTextCharsLength(); i++) {
    ...
    showStatus() {
        ...
        for (let i = 0; i < this.getStateCharsLength(); i++) {
    ...
    showTextWithPoints(text, points, persistenceMillis = DEFAULT_SHOW_TEXT_WITH_POINTS_PERSISTENCE_MILLIS, callback) {
        ...
        for (let i = 0; i < this.getTextWithPointsCharsLength(); i++) {
    ...
    drawTextState() {
        if ((this.textQueue.length > 0)) {
            ...
            if (!this.fixedPart) {
                ...
                if (!this.textArrayIsOnlyPersistence() && !this.onlyPersistenceTimerStarted) {
...
export function createBonusStageStatusBanner(stateStage) {
    return new StageStatusBanner(341, 380, BANNER_TYPE.BONUS_STAGE, stateStage);
}

export function createStageStatusBanner(stateStage) {
